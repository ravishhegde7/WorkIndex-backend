    ...
    if (!messages.length && Chat) {
      try {
        ...
        if (chat && chat.messages && chat.messages.length) {
          // Populate sender if it's an ObjectId
        ...
        if (chat) {
          return res.json({ success: true, messages: messages, chat: { expert: chat.expert, client: chat.client, request: chat.request } });
        }
      } catch(e) {}
    }
