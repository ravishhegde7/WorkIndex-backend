...
router.put('/requests/:id', protect, async (req, res) => {
  try {
    ...
    var request = await Request.findByIdAndUpdate(
      req.params.id,
      { title, description, status, creditsRequired },
      { new: true }
    );
    if (!request) return res.status(404).json({ success: false, message: 'Post not found' });
...
router.post('/users/:id/dm', protect, async (req, res) => {
  try {
    ...
    // Find or create an admin-direct chat for this user
    // We use a special adminDM flag / or find existing
    if (Chat && Message) {
      // Look for an existing admin DM chat
      var chat = await Chat.findOne({ adminDM: true, $or: [{ expert: req.params.id }, { client: req.params.id }] });
      if (!chat) {
        // Create a new admin DM thread
        chat = await Chat.create({
          adminDM: true,
          expert: user.role === 'expert' ? user._id : null,
          client: user.role === 'client' ? user._id : null,
          lastMessage: message.substring(0, 100),
          updatedAt: new Date()
        });
      }
      // Create the message from admin
      await Message.create({
        chat: chat._id,
        sender: null, // null = admin
        senderRole: 'admin',
        text: message,
        isAdminMessage: true
      });
